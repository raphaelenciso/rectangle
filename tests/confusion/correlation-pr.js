...
function generateCorrelationResults(algorithm = "rectangle") {
  ...
  keys.forEach((key, j) => {
    plaintexts.forEach((plaintext) => {
      ...
      ciphertexts.forEach((ciphertext, i) => {
        const corRes = pearsonCorrelation(plaintextBin, ciphertext);
        correlationResults[i].push(Math.abs(corRes));
      });
  ...
  for (let i = 0; i < correlationResults.length; i++) {
    let total = 0;
    for (let j = 0; j < correlationResults[i].length; j++) {
      total += correlationResults[i][j];
    }
    avgPerRound.push(total / correlationResults[i].length);
  }

  console.log(avgPerRound);
}
...
generateCorrelationResults();
// console.log(correlationResults[0].length);
