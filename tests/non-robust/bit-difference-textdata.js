...
const testCase = [
  ...
  ciphertextAsciiToDecimal("rectangle cipher"),
];
...
function runAlgo(algo) {
  const hdk = runTestCase(testCase, 1.1, algo);
  const ldk = runTestCase(testCase, 1.1, algo);
  const rk = runTestCase(testCase, 1.1, algo);
