...
const { bitIndTemplate } = require("../../modules/bit-difference-template");
...
function runTestCase(testCases, keys = 0, algorithm = "rectangle") {
  ...
  testCases.forEach((testCase) => {
    ...
    for (let i = 0; i < roundkeys.length - 1; i++) {
      const count = countBitDifferences(roundkeys[0], roundkeys[i + 1]);
      sumOfBitDiff += count;
    }
...
const templateString = bitIndTemplate(rData, tdData, mkData, mData);
console.log(templateString);

fs.writeFileSync(
  // "../../" +
  "results/non-robust/bit-independence-analysis.txt",
  templateString
);
