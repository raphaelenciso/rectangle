...
class Rectangle {
  constructor(plainText, key) {
    ...
    this.length = 128; // Length of the output bit sequenc
  }
  ...
  generateRoundKey(final = false) {
    ...
    if (!final) {
      ...
      this.seed = this.key[0];
      var bbsOuput = this.blumBlumShub(this.seed, this.p, this.q, this.length);
      for (let i = 0; i < 4; i++) {
        this.key[i] ^= bbsOuput[i];
      }
    }
  ...
  NonceXOR(addition = true) {
    ...
    const nonceValue = new Uint16Array([0x766c, 0x7264, 0x206e, 0x6373]);
    for (let i = 0; i < 4; i++) {
